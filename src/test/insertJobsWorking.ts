...
async function readJobsFromFile(filePath: string): Promise<Company[]> {
...
async function insertJobsIntoDB(connection:Connection,jobs: Job[], companyName: string, companyLogo: string, companyURL: string) {
...
`;
    ...
    const slugfromJobs = await jobs.map(job => `'${processHref(job.job_detail, companyName)}'`).join(',');
    console.log("slugfromJobs: " + slugfromJobs);

    for (const job of jobs) {
        const slug = processHref(job.job_detail, companyName);
        ...
        if (exists) {
            ...
            const [result]: any = await connection.execute(insertJobQuery, [job.jobTitle, slug, type, address, "USD", "Intermediate", experience, category_id, job.description, "Monthly", companyName, salaryRange, job.shortdesc, metadata, scrapped_info]);
...
export async function insertDataToDB() {
    ...
    const filePath = 'ycombinatorjobdetails.json';
    const companies = await readJobsFromFile(filePath);
    //connection establish
    const connection = await dbInstance.getConnection();
    try {
        for (const company of companies) {
            await insertJobsIntoDB(connection,company.jobs, company.name, company.logo,company.company_url);
        }
